...
const moduleEntries: ReadonlyArray<ModuleEntry> = [
  ...
  },
  {
    ...
    route: "/tools",
    shortcut: "-",
    summaryVi: "Hộp công cụ dev hằng ngày, 20 utility chia 5 nhóm, chạy hoàn toàn client-side, không gửi dữ liệu ra server.",
    capabilitiesVi: [
      "Encode/Decode: JSON formatter, Base64, URL encode, HTML entities, JWT decoder (header + payload), Hash SHA-1/256/512.",
      "Generators: UUID v4 + ULID (sinh hàng loạt), password generator (length 6-64, charset configurable, đo entropy), Lorem ipsum VN/EN (word/sentence/paragraph), Slug VN-aware (bỏ dấu tiếng Việt → kebab-case).",
      "Text: diff 2 đoạn (LCS, highlight thêm/xoá/giữ), word/char/byte counter + reading time, Markdown live preview, case convert (camelCase/PascalCase/snake_case/kebab-case/CONSTANT/Title).",
      "Code & Design: Regex tester (pattern + flags + matches), Cron parser giải nghĩa + 5 lần chạy kế tiếp theo giờ Việt Nam, number base bin↔oct↔dec↔hex, color picker HEX↔RGB↔HSL + Tailwind class + CSS var, WCAG contrast checker AA/AAA cho text thường + lớn.",
      "Time: timestamp converter Unix↔ISO↔giờ Việt Nam↔Bangkok↔relative.",
    ],
    howVi: "Mở /tools, các tool nhóm theo Encode/Generate/Text/Code & Design/Time. Bấm tab để chuyển. Nhập dữ liệu, kết quả update realtime. Có nút Copy mọi kết quả.",
  },
  {
    title: "Calculators",
    status: "live",
